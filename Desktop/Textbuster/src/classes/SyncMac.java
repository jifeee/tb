...
public class SyncMac  {
	...
	public void sendMac (Context ctx, String mac) {

		this.ctx = ctx;
		this.mac = mac;
		
		myUserStatus = Constants.myUserStatus;
		imei = Constants.imei;								//Imei should be set to Constants & Sharedprefs from somewhere else
		token = myUserStatus.getToken();
		
		TelephonyManager tm = (TelephonyManager)ctx.getSystemService(ctx.TELEPHONY_SERVICE);		//right now we get it here
		imei = tm.getDeviceId();
		
		SendMacTask smt = new SendMacTask();
    	
    	smt.execute(myUserStatus);
	...
	public class SendMacTask extends AsyncTask<UserStatus, String, UserStatus> {
	    ...
	    protected UserStatus doInBackground(UserStatus ... myUserStatus) {
	          ...
	          try{
	        	...
	        	JSONObject json = new JSONObject();
	        	String url = "http://textbuster.mobilezapp.de/api/textbusters/create/";
	        	url = url.concat(imei + "/");
	        	url= url.concat(mac + "/");
