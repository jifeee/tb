 ...
 */
navigator.map.initGoogleMaps = function(){
	navigator.map.gmap = document.createElement('div');
	navigator.map.gmap.style.width = '100%';
	navigator.map.gmap.style.height = '100%';
	navigator.map.gmap.setAttribute('id', 'googlemap');
	navigator.map.el.appendChild(navigator.map.gmap);
	
    var latlng = new google.maps.LatLng(navigator.map.lat, navigator.map.lng);
    ...
    var options = {
      ...
      mapTypeControllOptions: {
		mapTypeIds : new Array('HYBRID', 'ROADMAP'),
    ...
    navigator.map.gmap = new google.maps.Map(navigator.map.gmap, options);
}
