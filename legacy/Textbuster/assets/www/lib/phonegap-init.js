...
function initPhonegap(){
	if( navigator.userAgent.match(/Android/) )
		document.addEventListener("deviceready", function() { console.log('Phonegap ready'); } , false);
	else
		setupPhonegapDummy();
}
...
function setupPhonegapDummy(){
	...
	// Create 'navigator.service' dummy system
	navigator.service = new Object();
	
	navigator.service.running = true;
	navigator.service.start = function(onsuccess, onfailure){ navigator.service.running = true; if(onsuccess) onsuccess(); }
	navigator.service.stop = function(onsuccess, onfailure){ navigator.service.running = false; if(onsuccess) onsuccess(); }
	navigator.service.check = function(onsuccess, onfailure){ if(onsuccess) onsuccess(navigator.service.running); }
	navigator.service.send = function(message, onsuccess, onfailure){ if(onsuccess) onsuccess(navigator.service.running); console.log('Send to service: ' + message); }
	
	// Create 'navigator.bluetooth' dummy system
	navigator.bluetooth = new Object();
	navigator.bluetooth.isOn = false;
	
	navigator.bluetooth.start = function(){
		navigator.bluetooth.isOn = true;
		navigator.bluetooth.onCallback();
		
		if(navigator.bluetooth.foundCallback) navigator.bluetooth.foundCallback = navigator.bluetooth.foundCallback;		
		if(navigator.bluetooth.scanStartCallback) setTimeout(navigator.bluetooth.scanStartCallback, 200);
		if(navigator.bluetooth.foundCallback) setTimeout(function() { navigator.bluetooth.foundCallback([["11:22:33:44:55:66", "myPhone"]]); }, 1000);
		if(navigator.bluetooth.foundCallback) setTimeout(function() { navigator.bluetooth.foundCallback([["AA:BB:CC:DD:EE:FF", "Textbuster 4351"]]); }, 3000);
		if(navigator.bluetooth.foundCallback) setTimeout(function() { navigator.bluetooth.foundCallback([["5A:67:C0:03:DE:25", ""],["00:47:E3:45:EE:01", "Bluetooth Mouse"]]); }, 11000);
		if(navigator.bluetooth.scanStopCallback) setTimeout(navigator.bluetooth.scanStopCallback, 12200);
	}
	
	navigator.bluetooth.stop = function(){
		navigator.bluetooth.isOn = false;
		navigator.bluetooth.offCallback();
	}
	
	navigator.bluetooth.state = function(){
		navigator.bluetooth.stateCallback(navigator.bluetooth.isOn);
	}
